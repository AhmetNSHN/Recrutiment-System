...
class jobDetails(tk.Frame):
    def __init__(self, parent, job_title, l_mode):
        tk.Frame.__init__(self, parent)
        self.parent = parent
        self.i18n = I18N(l_mode)
        conn = sqlite3.connect("database.db")
        c = conn.cursor()
        c.execute("UPDATE jobs SET view = view + 1 WHERE  job_title=?", (job_title,))
        c.execute(
        ...
        c = conn.cursor()
        c.execute(
            "SELECT username  FROM applicants INNER JOIN jobs ON jobs.job_id = applicants.job_id WHERE job_title=? ",
            (job_title,))
        results = c.fetchall()
        ...
        applicants = tk.Label(f_bottom, text=self.i18n.applicants, font=(None, 20, "bold"))
        applicants.grid(column= 0, row=2, columnspan=2)
        j=0
        self.candidate_list = []
        for i in results:
            self.candidate_list.append(tk.Label(f_bottom, text= i[0], font=(None, 12)))
            self.candidate_list[j].grid(column=0, row=3+j, columnspan=2)
            self.candidate_list[j].bind("<Button-1>", lambda n=i[0]: parent.goto_profilepage(n))
            j+=1

        f_bottom_r = tk.Frame(f_bottom)
        f_bottom_r.grid(column=0, row=j+4, columnspan=2, pady=20)
